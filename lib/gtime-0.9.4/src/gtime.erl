...
%%-----------------------------------------------------------------------------
%% @spec gsecs2gdate(Gsecs::greg_secs()) -> greg_secs()
%% @doc Strip the non-date seconds (hh-mm-ss) from greg_sec()
%% @end
%%-----------------------------------------------------------------------------
gsecs2gdate(Secs) ->
...
%%-----------------------------------------------------------------------------
%% @spec gdate2date(GDate::greg_secs()) -> date()
%% @doc Converts a gdate to an Erlang date tuple.
%% @end
%%-----------------------------------------------------------------------------
gdate2date(Secs) ->
...
%%-----------------------------------------------------------------------------
%% @spec date2gdate(Date::date()) -> greg_sec() 
%% @doc Convert a {Y,M,D} tuple into Gregorian seconds.
%%-----------------------------------------------------------------------------
date2gdate(YMD) ->
    calendar:datetime_to_gregorian_seconds({YMD, {0,0,0}}).

%%-----------------------------------------------------------------------------
%% @spec datetime2gdate({Date::date(), Time::time()}) -> greg_secs() 
%% @doc Convert a {{Y,M,D},{HH,MM,SS}} tuple into Gregorian seconds.
%%-----------------------------------------------------------------------------
...
%%           rfc1123   -> "DOW, DD MOY YYYY HH:MM:SS" *
%% </pre>
%% Note    : the YYYY part can be 1+ chars <br />
%% Note    : * For a valid rfc1123, append your own time-zone
%%-----------------------------------------------------------------------------
gtostr(undefined, _) -> "-";
gtostr(Secs, date) ->
...
gtostr(Secs, rfc1123) ->
	...
	calendar:gregorian_seconds_to_datetime(Secs),
    DayOfWeek = day_abbr_str(calendar:day_of_the_week(Date)),
    lists:flatten(io_lib:format("~s, ~.2.0w ~s ~b ~.2.0w:~.2.0w:~.2.0w GMT",
                                [DayOfWeek, Day, month_abbr_str(Month),
                                 Year, Hour, Minute, Second])).

%%-----------------------------------------------------------------------------
%% @spec strtog(Date::string()) -> {ok, greg_secs()} | error
%% @doc Convert a date string to gregorian seconds
%% <pre>
%%      Date = string() on the "YYYY-MM-DD" | "YYYY-MM-DD HH:MM:DD" format
%%             note: H,M,D have no size limit so either one or two chars
%%             will work (if the num is lesser than 10)
%% </pre>
%% @end
%%-----------------------------------------------------------------------------
strtog(Date) ->
...
month_abbr_str(1) -> "Jan";
month_abbr_str(2) -> "Feb";
month_abbr_str(3) -> "Mar";
month_abbr_str(4) -> "Apr";
month_abbr_str(5) -> "May";
month_abbr_str(6) -> "Jun";
month_abbr_str(7) -> "Jul";
month_abbr_str(8) -> "Aug";
month_abbr_str(9) -> "Sep";
month_abbr_str(10) -> "Oct";
month_abbr_str(11) -> "Nov";
month_abbr_str(12) -> "Dec".
